...
class EditDayFragment : Fragment() {
    ...
    private fun incorrectDateFormatDialog() {
        val builder = AlertDialog.Builder(requireContext())
        builder.setTitle(getString(R.string.incorrect_date_format))
        builder.setMessage(getString(R.string.incorrect_date_format_instructions))
        builder.setPositiveButton(getString(R.string.ok)) { _, _ ->
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        var currentDateOfEntry = displayFormat.format(passedFormat.parse(date))
        ...
        if (savedInstanceState != null) {
            ...
            lifecycleScope.launch {
                val database = AppDatabase.getInstance(requireContext())
                val databaseDate = databaseFormat.format(displayFormat.parse(currentDateOfEntry))
        ...
        buttonEditDay.setOnClickListener {
            if (personId != -1) {
                lifecycleScope.launch {
                    ...
                    try {
                        val newDatabaseDate =
                            databaseFormat.format(displayFormat.parse(newDate)).toString()
                                ...
                                try {
                                    ...
                                    if (stressScore in 1..10) {
                                        val currentDatabaseDateOfEntry =
                                            databaseFormat.format(displayFormat.parse(currentDateOfEntry)).toString()
